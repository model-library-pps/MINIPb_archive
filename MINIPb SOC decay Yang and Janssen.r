...
fCremaining<-function(t=time, Cin=Cin, R=R, S=S, Temp=Temp){
  ...
  km <- Cin/CEQ
  MRT <- 1/km
  #SOC measured in the field is typically before application and hence represent the amount present after one year. 
...
SoilSOC<-function(SOCini=0, Rsoc=R, Ssoc=S, CinAnn=CinAnn, R=R, S=S, Temp=Temp){
  #identify the number of inputs and number of years
  ncomp<-ncol(CinAnn)
  ny<-nrow(CinAnn)
  ...
  #Determine the amount of SOC remaining of the initial amount of SOC 
  ...
  for(y in years){
    ...
    for(comp in components){
      ...
      #CinAnn[y,comp]: annual input for component in year y 
      #Determine how much is for all years after y
      CompC <- fCremaining(t=seq(1,ny+1-y,1), Cin=CinAnn[y,comp], R=R[comp],S=S[comp],Temp=Temp)$Ct
      #Add to SoilC
      SoilC[y:ny] <- SoilC[y:ny] + CompC
    }
#    SoilC<-rbind(SoilC, soilc)
  }
    
#  SC<-colSums(SoilC)
  return(list(YEARS=years, SOC=SoilC))
}
...
#To detemine total amounts of C in rooted topsoil
#Rooting depth
RD <- 0.2
...
par(mfcol=c(2,2),mar=c(1.5,1.5,0.5,0.5), oma=c(2,2,0.5,0.5))
...
lines(x,fCremaining(t=x, Cin=1,R=1.2,S=0.8,Temp=9)$k,col="brown",lty=4,lwd=2.0)
...
legend("topright",
       legend=c("Glucose","cellulose","Stalks/straw","Ryegrass","FYM"),
       lty=c(1,2,4,5),
       col=c("blue","red","brown","green","black"),
       lwd=c(2.0,2.0,2.0,2.0),
       bty="n")

plot(x,fCremaining(t=x, Cin=1,R=2.04,S=0.86,Temp=9)$Ct,ylim=c(0,1.0),
...
plot(x,fCremaining(t=x, Cin=1,R=2.04,S=0.86,Temp=18)$Ct,ylim=c(0,1.0),xlab="Time, y", ylab="SOC remaining,kg/kg",lty=1,lwd=2.0,type="l",col="blue")
...
mtext("SOC remaining,kg/kg                               k                  ", 
      side =2, outer  = TRUE, line = 0.5)
mtext("Time, y", side = 1, outer  = TRUE, line = 0.5)
